...
const std::string derraria_block_textures_png="derraria blocks V0.3.png";
const std::string derraria_player_stuff_textures_png="derraria player stuff V0.3.png";
...
int main(){
	...
	while(window.isOpen()){
		...
		if(sf::Keyboard::isKeyPressed(sf::Keyboard::Key::Q))
			for(int i=0;i<6;i++){ for(int j=0;j<6;j++){ loaded_chunks[i][j]=genChunk((i+(player.player.getPosition().x/16)/8)-3,(j+(player.player.getPosition().y/16)/8)-3,0); } }
		
		std::cout<<"X:"<<player.player.getPosition().x/8<<" Y:"<<player.player.getPosition().y/8<<" X:"<<loaded_chunks[0][0].data[0][0].x<<" Y:"<<loaded_chunks[0][0].data[0][0].y<<"\n";
